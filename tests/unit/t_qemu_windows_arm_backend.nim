...
# Runner-Fleet-M3-ARM-Wave MA3 gate: the "QemuWindowsArmBackend golden
# build" and "Golden build space precondition" suites that used to live here
# were MOVED, intact, to tests/unit/t_qemu_windows_arm_golden_build.nim so a
# grep for the gate name `t_qemu_windows_arm_golden_build` lands on every one
# of its assertions. Nothing was dropped or weakened in the move.
