...
[Backends](/guides/backends)); the examples below use Lima or Incus because they
...
See [Driving a VM from code](/guides/driving-a-vm) for the ephemeral lifecycle and
the JIT cloud-init injection seam, and the [CLI reference](/reference/cli-reference)
for every flag.
...
Continue in [Driving a VM from test / harness code](/guides/driving-a-vm).
