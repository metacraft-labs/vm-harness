---
title: Authoring a guest recipe
section: guides
order: 3
slug: guest-recipes
---
...
- `$VMH_RECIPES_DIR/<id>` — operator escape hatch,
- `<cwd>/guest-recipes/<id>` — running from a repo checkout,
- `<exe-dir>/../guest-recipes/<id>` and `<exe-dir>/../../guest-recipes/<id>`,
- `<exe-dir>/../share/vm-harness/guest-recipes/<id>` — the Nix-packaged layout.
...
- Is parameterized by `VMH_*` environment variables, each with a sensible
  ...
  the [Parameters catalog](/reference/parameters)).
...
[Parameters catalog](/reference/parameters).
