---
title: Backends: setup and caveats
section: guides
order: 2
slug: backends
---
...
- An alternative Windows-on-ARM path driven directly through `qemu-system-aarch64`
  ...
  catalog](/reference/parameters). Has a dedicated `prune` scope for its per-job state
  directory.
...
`qemu-windows-arm` backends (or `all`). See the [CLI reference](/reference/cli-reference#prune).
