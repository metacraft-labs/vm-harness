...
1. the **runner-image recipe seams** (`VMH_RUNNER_*` and friends) that shape the
   `vmh-linux-runner` Incus image, and
2. the **GARM Incus provider options** (`incus*`) a fleet operator sets to grant
...
defaults are the supported values (documented in [Driving a VM from code](./driving-a-vm.md)
