...
not target microVM-grade sub-second cold starts. See `../design.md` §1 for the
full in-scope / out-of-scope statement.
...
methods (defined in `src/vm_harness/types.nim`, mirrored from `../design.md`
§3.2) are:
...
guest OS) pair. The dispatch table (`selectBackendId` in `types.nim`,
`../design.md` §6) is:
...
per-backend transport details, and design references — read
[`../design.md`](../design.md).
