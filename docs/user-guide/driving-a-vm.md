...
the [CLI reference](./cli-reference.md)), or import the Nim library and call the
...
This is the lifecycle from `../design.md` §3.3: `provisionBaseline` once, then
`revertToBaseline` / exec / `stopAndCleanup` per gate.
...
consumes it. See [`../ephemeral-lifecycle-and-cleanup.md`](../ephemeral-lifecycle-and-cleanup.md)
and [`../per-backend-notes/incus.md`](../per-backend-notes/incus.md) for the
